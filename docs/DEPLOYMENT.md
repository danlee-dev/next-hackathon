...
- Supabase ref: `rownzmepduqeezanhicg` (URL 의 `*-ref-*` 부분)
...
## STEP 6 — Railway 환경변수 (Backend)

1. https://railway.app/dashboard → backend 서비스 클릭
2. 상단 **Variables** 탭
3. **Raw Editor** 클릭 (효율적)

4. *모든 키 값* 은 로컬 `backend/.env` 파일에 이미 채워져 있음. 그 파일을 통째로 열어 복사 → Railway Raw Editor 에 붙여넣기. 단 다음 *3 가지* 만 변경:
...
4. 저장 → Railway 자동 재배포.
5. 약 1분 후 https://trustpitch-frontend-production.up.railway.app/healthz 접속 → 다음과 같이 보여야 정상:
   ```json
...
```bash
# backend healthz
curl -s https://trustpitch-frontend-production.up.railway.app/healthz | python3 -m json.tool
...
기대:
- backend: `{"ok": true, "supabase": true, "openai": true, "demo_mode": false}`
- frontend: `HTTP/2 200`
